...
class ProductionConfigTests(unittest.TestCase):
    def test_production_rejects_placeholder_secret(self):
        config = Settings(_env_file=None, app_env="production", allowed_origins="https://console.example.com")
        with self.assertRaisesRegex(RuntimeError, "JWT_SECRET"):
            config.validate_runtime()

    def test_production_accepts_explicit_secure_origin(self):
        config = Settings(_env_file=None, app_env="production", jwt_secret="a-secure-random-value-with-more-than-32-characters", allowed_origins="https://console.example.com")
        config.validate_runtime()
