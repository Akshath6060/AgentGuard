...
async def seed():
    ...
    for i in range(48):
        ...
        if decision=="review" and i<32:
            await db.approvals.update_one({"transaction_id":txid},{"$set":{"approval_id":f"apr_hist_{41000+i}","transaction_id":txid,"workspace_id":WS,"status":"pending","reason_codes":[reason],"requested_at":created,"expires_at":t+timedelta(hours=24+i),"decided_by":None,"decided_at":None,"comment":None,"version":1,"created_at":created}},upsert=True)
